...
const CarouselA = (props) => {
  const {slides = [], img_width = IMG_WIDTH, img_height = IMG_HEIGHT, visibleImages = VISIBLEIMAGES, duration = DURATION, autoNext = false, timeForNext = 3000} = props;
  ...
  const constructVisibleItemsProps = () => {
    ...
    while(timesToIterate < visibleImages ) {
      ...
      if (timesToIterate < elementsInRight) {
        ...
        currImgIndex = nextIndex > -1 ? nextIndex : slides.length - Math.abs(nextIndex); // Gives the rightmost elemnt in first iteration and then the subsequent elements down the iteration
        ...
        if (curr_center_copy + 1 >= slides.length) { // to maintain cyclic carousel
          curr_center_copy = 0;
        } else {
          curr_center_copy++;
        }
  ...
  const cycleToNextImage = (actual) => {
    if (visibleItemsProps.order.indexOf(currMiddleImgRef.current) > visibleItemsProps.order.indexOf(actual)) {  // Right side image click
      currMiddleImgRef.current = currMiddleImgRef.current - 1 > -1 ? currMiddleImgRef.current - 1 : slides.length - 1; // Right side image click
      setCurrFirstImg(currMiddleImgRef.current);
    } else {  // Left side image click
      currMiddleImgRef.current = (currMiddleImgRef.current + 1) < slides.length ?  (currMiddleImgRef.current + 1) : 0; // Conditions to handle cycle
      setCurrFirstImg(currMiddleImgRef.current);
    }
  }
  ...
  useEffect(() => {
    if (autoNext) {
      setInterval(() => {
        const nextImg = currMiddleImgRef.current + 1 < slides.length ?  currMiddleImgRef.current + 1 : 0;
  ...
  const loadCarousel = () => {
    return (
      ...
      {
        slides.map(({large_url, url, id}, index) => {
